var HelpBox = React.createClass({
    render: function() {
        return (<div key={2}>
            <h4>NPM setup</h4>
            <p>By default are artifacts downloaded with TLS from  <a href="http://registry.npmjs.org">registry.npmjs.org</a>.</p>
            <p>To achieive a MITM proxy there are some options:
                <ul>
                    <li>disable certificate verification with <span className="raw">npm config strict-ssl false</span> or <span className="raw">npm --strict-ssl false install</span></li>
            ...
            <p>By default are artifacts downloaded from Maven Central at <a href="http://repo.maven.apache.org">repo.maven.apache.org</a>. Version 3.2.6
                and later will use TLS for acessing the site, earlier versions raw HTTP.</p>
            <p>A succesful TLS setup requires injecting the MITM certification in the <span className="raw">$JAVA_HOME/jre/lib/security/cacert</span> file of the JVM used to run Maven:</p>

            <div className="raw">
                keytool -storepass changeit -noprompt -import -alias docker-mitm-proxy -keystore cacerts -file \ <br/>
            ...
            <h4>CURL</h4>
            <p>When accessing https URLs with curl one can either:
                <ul>
                    <li>disable certificate verification with <span className="raw">-k</span>/<span className="raw">--insecure</span> options</li>
                    <li>refer to the proxy CA certificate with <span className="raw">--cacert rootCA.pem</span></li>
                </ul>
            </p>
